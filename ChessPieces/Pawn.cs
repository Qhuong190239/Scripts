...
public class Pawn : ChessPiece
{
    ...
    public override SpecialMove GetSpecialMoves (ref ChessPiece[,] board, ref List<Vector2Int[]> moveList, ref List<Vector2Int> availableMoves)
    {
        ...
        if(moveList.Count > 0)
        {
            ...
            if(board[lastMove[1].x, lastMove[1].y].type == ChessPieceType.Pawn) // If last piece moved was Pawn
            {
                if(Mathf.Abs(lastMove[0].y - lastMove[1].y) == 2)               // If last move was a +2 in either direction
                {
                    if (board[lastMove[1].x, lastMove[1].y].team != team)       // If last move from another team
                    {
